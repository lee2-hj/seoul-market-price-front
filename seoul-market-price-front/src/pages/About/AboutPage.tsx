...
export default function AboutPage() {
  return (
    <div className="tw-scope mx-auto flex w-[min(1280px,calc(100%-32px))] flex-col gap-4 py-10 sm:w-[min(1280px,calc(100%-48px))] sm:gap-5 sm:py-12 lg:gap-6 lg:py-16">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl">싸.부란?</CardTitle>
          <CardDescription className="text-sm">
            서울시 부동산 실거래가 및 매물 시세 정보를 한눈에 비교할 수 있는 서비스입니다.
          </CardDescription>
        </CardHeader>
        <CardContent className="text-sm text-muted-foreground">
          지역별 실거래가 시세 조회, 자치구별 가격 비교, 스마트 추천 기능을 통해 더
          합리적인 부동산 선택을 할 수 있도록 돕습니다.
        </CardContent>
      </Card>
    </div>
  );
