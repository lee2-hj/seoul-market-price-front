...
type QnaRawDetailType = {
  ...
  files?: QnaAttachmentFileType[] | null;
  attachedFiles?: QnaAttachmentFileType[] | null;
  fileList?: QnaAttachmentFileType[] | null;
  attachmentList?: QnaAttachmentFileType[] | null;
};

type QnaFullResponseType = {
  detail: QnaRawDetailType;
  attachments: AttachmentResponse[];
};

type QnaDetailViewModelType = {
  id: number;
  title: string;
  author: string;
  content: string;
  ...
  isAnswered: boolean;
  attachments: QnaAttachmentFileType[];
  rawDetail: QnaRawDetailType;
};
...
export default function QnaDetailPage() {
  ...
  /* TanStack Query: QnA 상세 조회 & Select 뷰모델 변환 */
  const { data: qnaViewModel, isLoading, isError, error } = useQuery<QnaFullResponseType, Error, QnaDetailViewModelType>({
    queryKey: ["qnaDetail", id],
    queryFn: async (): Promise<QnaFullResponseType> => {
      if (!id) throw new Error("잘못된 게시글 번호입니다.");
      return fetchQnaDetailApi(id);
    },
    select: (fullDetail: QnaFullResponseType): QnaDetailViewModelType => {
      ...
      if (singleName && !list.some((existing) => existing.name === singleName)) {
        list.push({ id: 1, name: singleName, url: singleUrl || undefined });
      }

      const answerText = post.answerContent || post.answer || "";
      return {
        id: post.id,
        title: post.title || "제목 없음",
        author: post.writerName || post.writerLoginId || "익명",
        content: post.questionContent || post.content || "내용이 없습니다.",
        answer: answerText,
        views: post.viewCount ?? post.views ?? 0,
        formattedDate: formatDate(post.createdAt),
        isAnswered: Boolean(answerText),
        attachments: list,
        rawDetail: post,
      };
    },
  ...
  const isMyPost = useMemo(() => {
    if (!currentUser || !qnaViewModel?.rawDetail) return false;
    const post = qnaViewModel.rawDetail;
    ...
    const postKeys = [post.writerLoginId, post.writerName, postRecord?.authorId, postRecord?.userId]
      .filter(Boolean)
      .map((s) => String(s).trim().toLowerCase());
    return userKeys.some((uk) => postKeys.some((pk) => uk === pk || uk.includes(pk) || pk.includes(uk)));
  }, [currentUser, qnaViewModel]);
  ...
  const handleEditQna = useCallback(() => {
  ...
  const handleDownloadAttachment = useCallback(async (file: QnaAttachmentFileType) => {
    try {
        ...
        <main className="py-8">
          <section className="min-w-0">
            ...
            {isLoading ? (
              ...
              <div className="p-12 text-center text-rose-600 font-bold bg-white rounded-2xl border border-rose-200">
              ...
              <div className="bg-white rounded-2xl border border-[#E2E8F0] p-6 shadow-sm">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <span
                      className={cn(
                        ...
                        qnaViewModel.isAnswered ? "bg-[#EBF5F8] text-[#0F766E] border border-[#7CC9D8]" : "bg-[#F5FAFC] text-[#6B7280] border border-[#DCE8ED]"
                      )}
                    >
                      {qnaViewModel.isAnswered ? "답변완료" : "답변대기"}
                    </span>
                  </div>
                ...
                {/* 첨부파일 영역 */}
                {qnaViewModel.attachments.length > 0 && (
                  <div className="mb-6 p-4 rounded-xl bg-[#F5FAFC] border border-[#DCE8ED]">
                    <div className="flex items-center gap-2 text-[13px] font-bold text-[#13202B] mb-2.5">
                      <Paperclip className="size-4 text-[#0F8AA8]" />
                      <span>첨부파일 ({qnaViewModel.attachments.length}개)</span>
                    </div>
                    <div className="flex flex-col gap-2">
                      {qnaViewModel.attachments.map((file, index) => (
                        <div key={index} className="flex items-center justify-between p-2.5 rounded-lg bg-white border border-[#E2E8F0] text-[13px]">
                          <div className="flex items-center gap-2 min-w-0">
                            <FileText className="size-4 text-[#64748B] shrink-0" />
                            <span className="font-medium text-[#13202B] truncate">{file.name}</span>
                            {file.size && <span className="text-[11px] text-[#94A3B8]">({(file.size / 1024).toFixed(1)} KB)</span>}
                          </div>
                          <Button
                            type="button"
                            variant="ghost"
                            ...
                            className="flex items-center gap-1 px-3 py-1.5 rounded-md bg-[#EBF5F8] hover:bg-[#DCE8ED] text-[#0F8AA8] text-[12px] font-bold transition-colors shrink-0 h-auto cursor-pointer"
                          >
                ...
                {/* 본문 내용 */}
                <div className="min-h-[180px] text-[15px] leading-relaxed text-[#334155] whitespace-pre-wrap py-2 mb-8">
                  {qnaViewModel.content}
                </div>

                {/* 관리자 답변 영역 */}
                {qnaViewModel.isAnswered && (
                  <div className="mt-8 pt-6 border-t border-[#E2E8F0] rounded-xl bg-[#EBF5F8]/50 p-5 border border-[#7CC9D8]/40">
                    <div className="flex items-center gap-2 mb-3">
                      <span className="px-2.5 py-0.5 rounded bg-[#0F766E] text-white text-[11px] font-bold">관리자 답변</span>
                      <span className="text-[12px] text-[#64748B]">답변 완료</span>
                    </div>
                    <div className="text-[14px] leading-relaxed text-[#0F766E] font-medium whitespace-pre-wrap">
                      {qnaViewModel.answer}
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between pt-6 border-t border-[#E2E8F0] mt-8">
                  <Button
                    ...
                    className="px-5 py-2.5 rounded-xl border border-[#DCE8ED] bg-[#F5FAFC] hover:bg-[#EBF5F8] text-[#13202B] text-[13px] font-bold transition-colors h-auto cursor-pointer"
                  >
                    목록으로
                  </Button>
                  ...
                  {canEditOrDelete && (
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        className="px-5 py-2.5 rounded-xl bg-[#0F8AA8] hover:bg-[#0D7893] text-white text-[13px] font-bold transition-colors h-auto cursor-pointer"
                      >
                        수정하기
                      </Button>
                      <Button
                        ...
                        className="px-5 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-[13px] font-bold transition-colors h-auto disabled:opacity-50 cursor-pointer"
                      >
