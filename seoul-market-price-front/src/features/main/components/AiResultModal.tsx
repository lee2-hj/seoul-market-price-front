import { ArrowRight, ArrowRightLeft, ChartNoAxesCombined, Lightbulb, Map, Search } from "lucide-react";
  ...
  return (
    <Dialog open={open && Boolean(result)} onOpenChange={(next) => { if (!next) onClose(); }}>
      {result && (
        <DialogContent
          showCloseButton
          className="flex h-[92vh] max-h-[96vh] w-full max-w-4xl flex-col gap-0 overflow-hidden rounded-3xl border border-[#DCE8ED] bg-white p-0 shadow-[0_30px_90px_rgba(7,29,42,0.30)]"
        >
          {/* 시각적으로는 아래 커스텀 h2가 제목 역할을 하므로, Radix 접근성 요구사항 충족용으로만 숨김 처리 */}
          <DialogHeader className="sr-only">
            <DialogTitle>{formatAiMoneyText(result.summary) || "AI 답변을 표시할 수 없습니다."}</DialogTitle>
          </DialogHeader>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-6 pb-6 md:px-10 md:pb-8">
            ...
            <h2 className="mb-0 mt-7 whitespace-pre-wrap break-words text-2xl font-black leading-9 tracking-[-0.03em] text-[#123047] md:text-4xl md:leading-[1.35]">{formatAiMoneyText(result.summary) || "AI 답변을 표시할 수 없습니다."}</h2>
