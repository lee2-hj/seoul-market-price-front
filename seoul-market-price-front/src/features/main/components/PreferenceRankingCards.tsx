      ...
      <CardHeader className="border-b border-[#E8EFF2] p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <CardTitle className="text-base font-black text-[#123047]">내 선호지역 거래량 TOP 5</CardTitle>
            <p className="mb-0 mt-0.5 text-xs text-[#6B7280]">법정동별 거래 건수 순위</p>
          </div>
        </div>
      </CardHeader>
      <CardContent className="p-4">
        {items.length === 0 ? (
          ...
          <ol className="m-0 divide-y divide-[#EDF2F4] p-0">
            {items.map((item) => (
              <li
                key={`${item.rank}-${item.districtName}-${item.dongName}`}
              ...
              >
                ...
                <span className="min-w-0">
                  <strong className="block truncate text-sm text-[#13202B]" title={`${item.districtName} ${item.dongName}`}>
                    {item.dongName}
                  </strong>
                  <span className="block truncate text-[11px] text-[#6B7280]">{item.districtName}</span>
                </span>
                <strong className="text-right text-sm tabular-nums text-[#123047] shrink-0">
                  {formatTradeCount(item.dealCount)}
                </strong>
              </li>
            ))}
          </ol>
    ...
    <Card className="h-full rounded-2xl border-[#DCE8ED] bg-white shadow-[0_3px_12px_rgba(18,48,71,0.05)]">
      <CardHeader className="border-b border-[#E8EFF2] p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <CardTitle className="text-base font-black text-[#123047]">내 선호지역 아파트 거래량 TOP 5</CardTitle>
          ...
          <ol className="m-0 divide-y divide-[#EDF2F4] p-0">
            {items.map((item) => (
              ...
              >
                <Rank value={item.rank} />
                <span className="min-w-0">
                  <strong className="block truncate text-sm text-[#13202B]" title={item.apartmentName}>
                    {item.apartmentName}
                  </strong>
                  <span className="block truncate text-[11px] text-[#6B7280]">
                    최근 {formatPriceInManwon(item.recentDealPrice)}
                  </span>
                </span>
                <strong className="text-right text-sm tabular-nums text-[#123047] shrink-0">
                  {formatTradeCount(item.dealCount)}
